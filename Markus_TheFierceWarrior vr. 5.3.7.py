...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 22 18:31:53 2019
...
while play: #hovedloop
    ...
    def movement(vel, cords, target):
        ...
        if not (x_cords == 0 or y_cords == 0):
            if abs(x // y) > 1:
                if not (abs(x / y) * vel / (abs(x / y)**2 + 1) ** 0.5) > abs(x):
                    x_cords = abs(x / y) * vel / (abs(x / y)**2 + 1) ** 0.5
                else:
                    x_cords = abs(x)
    
                if not (vel / (abs(x / y)**2 + 1) ** 0.5 > abs(y)):
                    y_cords = vel / (abs(x / y)**2 + 1) ** 0.5
    
                else:
                    y_cords = abs(y)
            else:
                if not (vel / (abs(y / x)** 2 + 1) ** 0.5) > abs(x):
                    x_cords = vel / (abs(y / x)** 2 + 1) ** 0.5
                else:
                    x_cords = abs(x)
    
                if not (abs(y / x) * vel / (abs(y / x)**2 + 1) ** 0.5) > abs(y):
                    y_cords = abs(y / x) * vel / (abs(y / x)**2 + 1) ** 0.5
                else:
                    y_cords = abs(y)
    
        return (cords[0] + x_cords * x_neg, cords[1] + y_cords * y_neg)
...
#            pygame.draw.rect(win, (255,0,0), self.hitbox,2) #Kan kommenteres ut
    ...
    class fireball(object):
        ...
        def move(self): #for fireball
            self.cords = (self.sx, self.sy)
            self.ext = self.cords
            self.cords = movement(self.vel, self.cords, (s_width/2, s_height/2))
            self.xdistance = self.cords[0]
            self.ydistance = self.cords[1]
...
#                self.okay = True
#
            self.x += self.xdistance - self.ext[0]
            self.y += self.ydistance - self.ext[1]
            self.sx += self.xdistance - self.ext[0]
            self.sy += self.ydistance - self.ext[1]
    ...
    class enemy(object): #klasse for fienden
        ...
        def move(self): #for enemy
            man.cords = (man.x, man.y)
            self.cords = (self.x, self.y)
            self.cords = movement(self.vel, self.cords, man.cords) #man.cords er en tuppel med x og y verdien til man
            self.x = self.cords[0]
            self.y = self.cords[1]
...
#                    self.y -= (self.vel ** 2 / 2) ** 0.5
    ...
    class Boss(object): #Klasse for bossen
        ...
        def attack(self): #Angrep
            ...
            if self.atkpattern == 1: #Tre angrepsmønstre | Her 
...
#                            button.move()
    ...
    if game_inits: 
        ...
        while run and man.health > 0:  # Hovedloop
            ...
            if keys[pygame.K_TAB]: #Dersom man klikker TAB vis Minimap
                ...
                if keys[pygame.K_l] and special_delay == 0: #Dersom man klikker L bruker man special angrep
                    man.special()
            ...
            if man.health == 0: #Stoper spillet dersom du dør og sender spiller til Game Over
                    enemies.clear()
                    run = False
    ...
    def draw_window(): #Tegner den siste  skjermene (Game Over og Credits)
        ...
        for button in buttons:
            win.blit(pygame.image.load("ButtonCredits-1.png"), (button.x, button.y)) #Tegner Credits-knappen
